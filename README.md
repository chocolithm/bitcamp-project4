...
[원인]
- OutputStream 사용 시. 기존에 송신한 내용을 캐시로 저장해 두었다가 재활용할 수 있음
...
# 7. 개선 필요사항
### 서버 멀티스레딩 이슈
- 게임을 시작할 때, 서버에서는 별도 스레드를 통해 게임 진행
- 하지만 현재 게임은 서버-클라이언트의 1대1 구조이므로, 스레딩 처리가 불필요
- 서버는 게임 중계를 담당하고, 클라이언트끼리 게임을 진행할 수 있도록 개선 필요

### 데이터 저장 이슈
- 현재 데이터 저장은 서버가 종료될 때 일괄 처리되기에 실시간 갱신 불가
- 향후 다중 클라이언트를 고려하면 실시간 처리가 가능한 DB로 전환 필요
